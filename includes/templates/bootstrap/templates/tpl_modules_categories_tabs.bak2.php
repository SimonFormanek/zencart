...
//	$i=0;
	// currently selected category
		echo '<li id="m' . $categories_tab->fields['categories_id'] . '">';
		...
		if((int)$cPath == $categories_tab->fields['categories_id']) 
		 echo '<span class="category-subs-selected"><b>'.$categories_tab->fields['categories_name'].'</b></span>';
		else 
		...
		{
			echo '<div id="mdiv">';
//			echo '<div id="mdiv' . $categories_tab->fields['categories_id'] . '">';
			while (!$subcategories_tab->EOF) 
			{
				$cPath_new = "cPath=".zen_get_generated_category_path_rev($subcategories_tab->fields['categories_id']);
				echo ' '.'<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '">'.$subcategories_tab->fields['categories_name'].'</a>';
