...
if (CATEGORIES_TABS_STATUS == '1') {
	echo '<div class="border-bottom d-none d-lg-block">';
...
$top = '<table class="mt"><tr>';
	...
	$top .=	 '<a href="' . basename($_SERVER['REQUEST_URI']) . '#m' . (int)$categories_tab->fields['categories_id'] .'">'; 
		...
		{
		  $style .= '#m' .  (int)$categories_tab->fields['categories_id'] . ':target,';
			...
			while (!$subcategories_tab->EOF) 
			{
				...
				$hidden .= '<a href="' . zen_href_link(FILENAME_DEFAULT, $cPath_new) . '">'.$subcategories_tab->fields['categories_name'].'</a> ';
...
//		echo '</li>';
	...
	echo $top . '</tr></table>' . PHP_EOL;
	echo preg_replace("/,$/", "", $style) . '{display:block}</style>' . PHP_EOL;
	echo  $hidden . PHP_EOL;
//	echo '</ul>';
	echo '</div>';
	echo '</div>';
